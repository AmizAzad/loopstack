...
## Concepts
- **Loop.** Every task runs observe → understand → act → verify → learn → adjust. Deploying does not end the loop: `/observe` feeds `/debug`, `/performance` and `/product`. Lessons are appended to `.loopstack/learnings.md`.
...
- `/swarm`: independent units run on parallel workers. loopstack refuses to swarm small or tightly coupled tasks.
